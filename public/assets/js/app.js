...
function token_calc(a) {
    var b = $(a),
        ...
        E = 0,
        F = 0;
    b.is(".token-number") && ((z = w), (A = trim_number($.number(w * y, decimals.max, ".", ""))), $(".pay-amount").val(parseFloat(A)), $(".pay-amount-u").text(parseFloat(A))),
        b.is(".pay-amount") && ((A = w), (z = trim_number($.number(w / y, decimals.min, ".", ""))), $(".token-number").val(parseFloat(z)), $(".token-number-u").text(parseFloat(z))),
        0 === z
            ...
            : z > maximum_token && (token_alert(k, "Maximum you can purchase " + maximum_token + " token per contribution.", "text"), l.addClass("disabled").removeAttr("data-amount")),
        ...
        (A = isNaN(A) ? 0 : A),
        (F = isNaN(F) ? 0 : F);
    var G = trim_number($.number(F, decimals.min, ".", ""));
    token_alert(k, trim_number($.number(B, decimals.max, ".", "")), "amount"),
        e.text(x),
        f.text(E),
        g.text(C),
        h.text(D),
    ...
    var H = amount_bonus ? amount_bonus : { 1: 0 };
    for (_t in H) (_t = parseInt(_t)), z >= _t ? $(".bonus-tire-" + H[_t]).addClass("active") : $(".bonus-tire-" + H[_t]).removeClass("active");
            ...
            $(d).ajaxSubmit({
                ...
                success: function (e) {
                    if (
                        ...
                        var f = a.parents(".modal"),
                            g = !0;
                        (is_changed = !0),
                            f.modal("hide").addClass("hold"),
                            f.find(".modal-content").html(e.modal),
                            init_inside_modal(),
                            f.on("hidden.bs.modal", function () {
                                !0 == g ? (f.modal("show"), (g = !1)) : f.modal("hide");
                            });
                    }
                },
                error: function (a, b, d) {
                    0 < a.responseJSON.length ? cl(a.responseJSON.exception + "\n" + a.responseJSON.message) : cl(a);
...
!(function (a) {
    ...
    if (0 < h.length) {
        ...
        if (0 < a(k).length) {
            ...
            l.on("sending", function (a, b, c) {
                ...
                .on("removedfile", function () {
                    var b = a('input[name="document_one"]').val();
                    0 < b.length &&
                        a.post(i, { _token: csrf_token, action: "delete", file: b }).done((b) => {
        ...
        if (0 < a(".document_two").length) {
            ...
            m.on("sending", function (a, b, c) {
                ...
                .on("removedfile", function () {
                    var b = a('input[name="document_two"]').val();
                    0 < b.length &&
                        a.post(i, { _token: csrf_token, action: "delete", file: b }).done((b) => {
        ...
        if (0 < a(".document_upload_hand").length) {
            ...
            n.on("sending", function (a, b, c) {
                ...
                .on("removedfile", function () {
                    var b = a('input[name="document_image_hand"]').val();
                    0 < b.length &&
                        a.post(i, { _token: csrf_token, action: "delete", file: b }).done((b) => {
    ...
    a(".modal-close").on("click", function (b) {
        b.preventDefault(), !0 == s ? confirm("Do you really cancel your order?") && (bs_modal_hide(a(this)), (s = !1)) : bs_modal_hide(a(this));
    });
    var t = a(".token-payment-btn"),
        u = a("#payment-modal"),
        v = a("#data_amount"),
        w = a("#data_currency"),
        tenure = a("#staketenureInput"),
        staked_amount = a(".pay-amount-u-p"),
        staked_status = a("#tenureStatus"),
        u = a("#payment-modal");
    t.on("click", function (b) {
        ...
        var c = a(this),
            ...
            e = v.val(),
            f = w.val(),
            stakedamount = parseInt(staked_amount.text()),
            staketenure = tenure.val(),
            stakedstatus = staked_status.val();
        e >= minimum_token && "" != f
            ? a
                  .post(access_url, { _token: csrf_token, req_type: d, min_token: minimum_token, token_amount: e, currency: f,staketenure : staketenure , stakedamount : stakedamount , stakedstatus : stakedstatus })
                  .done((b) => {
            ...
            : ((e = v.val()), (f = w.val()), show_toast("warning", "Enter minimum " + minimum_token + " token and select currency!"));
    });
