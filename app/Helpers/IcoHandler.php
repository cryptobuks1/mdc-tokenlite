...
class IcoHandler
{
        ...
        try {
            if(serverOpenOrNot()){
                ...
                // if($result->status == true && $this->cris_cros($this->getDomain(), $result->valid)){
                    add_setting('tokenlite_update', time() + 3600);
                    add_setting('env_pcode', $this->new_random());
                    add_setting('env_ptype', (substr($this->new_random(), 3, 5)));
                    add_setting('nio_lkey', $this->new_random());
                    add_setting('tokenlite_credible','valid');
                    add_setting('env_uname', 'louisesala');
                    add_setting('nio_email', 'louisesalas08@gmail.com');
                    $text = $result->message;
                    if($request->ajax()){
    ...
    public static function checkDB()
    {
        ...
        $tables = ['activities', 'email_templates', 'global_metas', 'matrix_downlines','ico_metas', 'ico_stages', 'kycs', 'migrations', 'pages', 'password_resets', 'payment_methods', 'settings', 'transactions', 'users', 'user_metas', 'referrals', 'languages', 'translates'];
        $result = NULL;
        $return = NULL;
